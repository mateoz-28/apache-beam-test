...
public interface Constants {
    String INPUT_FILE_PATH = "D:\\Documents\\FER\\5.godina\\10.semestar\\Diplomski rad\\Materijali\\Sensor Data.csv"; // Path to input .csv file
    long LINES = 5000000L; // Number of lines to read
    long ITERATIONS = 1L; // Number of iterations for reading input .csv file
    float PERCENTAGE = (float) 0.95; // Percentage of messages that pipeline has to read from Kafka producer (value must be less or equal to 1)
    long CONSUMER_POLL_MS = 5000; // Consumer poll time in [ms]
    String BOOTSTRAP_SERVERS =  "localhost:9092"; // Bootstrap server URL
    String INPUT_TOPIC_NAME = "input-topic-10"; // Name of Kafka input topic
